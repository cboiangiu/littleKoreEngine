...
Kore::mat4 TransformComponent::mat4()
{
    ...
    mat.Set(1, 0, scale.x() * (c2 * s3));
    mat.Set(2, 0, scale.x() * (c1 * s2 * s3 - c3 * s1));
    mat.Set(3, 0, 0.0f);

    mat.Set(0, 1, scale.y() * (c3 * s1 * s2 - c1 * s3));
    mat.Set(1, 1, scale.y() * (c2 * c3));
    mat.Set(2, 1, scale.y() * (c1 * c3 * s2 + s1 * s3));
    mat.Set(3, 1, 0.0f);

    mat.Set(0, 2, scale.z() * (c2 * s1));
    mat.Set(1, 2, scale.z() * (-s2));
